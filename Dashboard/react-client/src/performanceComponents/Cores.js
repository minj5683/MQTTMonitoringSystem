...
class Cores extends Component {
  ...
  componentDidMount() { //컴포넌트가 화면에 나타나게 되었을 때 호출
    ...
    makeChart.create("core2", "core2ChartHolder", makeChart.core2LayoutStr, this.chartData);
    makeChart.create("core3", "core3ChartHolder", makeChart.core3LayoutStr, this.chartData);
    makeChart.create("core4", "core4ChartHolder", makeChart.core4LayoutStr, this.chartData);
  }
  ...
  messageArrived(data) {  //차트 데이터 갱신
    ...
    let core4 = [];
    let date = [];
    for(let i=1; i<(dataArray.length-1 -1)/2 + 1; i++) {
        coresUtil.push(dataArray[i*2 -1 +1]);
    ...
    this.setChartData(dataLength, date, core1, "core1");
    this.setChartData(dataLength, date, core2, "core2");
    this.setChartData(dataLength, date, core3, "core3");
    this.setChartData(dataLength, date, core4, "core4");
  }


  setChartData(dataLength, date, coresUtil, chartId) {
    this.chartData = []; //기존 차트 데이터 삭제, 업데이트 될 차트 데이터로 갱신
    ...
    //document.getElementById("core1").setData(this.chartData);
  ...
  render() {
    ...
    return (
      <Fragment>
        ...
        <div style={style}>
            <div style={style1} id="core1ChartHolder"/>
            <div style={style1} id="core2ChartHolder"/>
            <div style={style1} id="core3ChartHolder"/>
            <div style={style1} id="core4ChartHolder"/>
        </div>
