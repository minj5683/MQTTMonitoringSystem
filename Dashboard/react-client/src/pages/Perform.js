...
import MosquittoPerform from '../performanceComponents/MosquittoPerform';
import PerformMenu from '../components/PerformMenu';
...
class Perform extends Component {
  ...
  componentDidMount() { //컴포넌트가 화면에 나타나게 되었을 때 호출
    ...
    mqtt.on('message', function(topic, message){
        ...
        else if(topic.toString().split('/')[1] === 'mosquitto') {
            this.setState({
                mosquittoChartData: message.toString()
            }, function () {
                //console.log(this.state.mosquittoChartData);
            });
        }
  ...
  render() {
    ...
    <MosquittoPerform chartData={this.state.mosquittoChartData}/>
